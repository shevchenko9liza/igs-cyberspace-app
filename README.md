...
### Стратегия привлечения: Short-Form Video

Подростки 11-18 лет живут в TikTok, YouTube Shorts и Reels. Это главный канал привлечения.

```
  TikTok / Reels / Shorts               CyberSpace
         |                                   |
  Короткий ролик 15-30 сек            Пользователь переходит
  "Застраховал телефон в игре,         по ссылке в био
   а потом его украли --                    |
   и страховка РЕАЛЬНО                 Проходит tutorial,
   сработала!"                          квиз, играет
         |                                   |
  Вирусный формат:                     Формируется понимание
  интрига + результат                  страхования + Brand
  = высокий retention                  Consideration
```

**Формат роликов:**
- **Hook (0-3 сек):** провокация -- "Тебе украли телефон. Что делать?"
- **Геймплей (3-20 сек):** запись экрана из CyberSpace -- покупка страховки, ЧП, выплата
- **Вывод (20-30 сек):** "Со страховкой заплатил 20% вместо 100%. А ты знал?"
- **CTA:** "Попробуй сам -- ссылка в био"

**Почему это работает:**
- Контент нативен для платформы (запись экрана игры = привычный формат)
- Образовательный контент с элементом интриги даёт высокий engagement
- Подросток сначала смотрит, потом играет сам -- двойное закрепление
- Низкий CAC: один вирусный ролик = тысячи переходов
