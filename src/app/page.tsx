...
import { Send, Gamepad2, MessageCircle, Shield, ShieldAlert, Wrench, Zap, Home } from 'lucide-react';
    ...
    return (
      <div style={{ ...S.page, alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <div style={{ textAlign: 'center', maxWidth: 340 }}>
          ...
          <h1 style={{ fontSize: 26, fontWeight: 900, color: '#a78bfa', marginBottom: 8 }}>CyberSpace</h1>
          <div style={{ fontSize: 14, color: '#94a3b8', marginBottom: 20, lineHeight: 1.7 }}>
            Привет! Добро пожаловать в CyberSpace. Дарю Смартфон.<br />
            Работай на нём, развивайся, покупай новую недвижимость, но <strong>берегись рисков!</strong>
          </div>
          <button onClick={completeOnboarding} style={{ width: '100%', padding: '14px 0', borderRadius: 12, fontSize: 16, fontWeight: 900, background: 'linear-gradient(135deg,#7c3aed,#5b21b6)', color: 'white', border: 'none', cursor: 'pointer', boxShadow: '0 0 30px rgba(124,58,237,0.5)' }}>
            Начать жизнь →
          </button>
        </div>
      </div>
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: '#0d0820', borderTop: '1px solid #7c3aed', overflow: 'hidden' }}>
        <div style={S.tabBar}>
          <button style={S.tab(tab === 'assets')} onClick={() => setTab('assets')}><Gamepad2 size={14} /> Имущество</button>
          <button style={S.tab(tab === 'real_estate')} onClick={() => setTab('real_estate')}><Home size={14} /> Недвижимость</button>
          <button style={S.tab(tab === 'mentor')} onClick={() => setTab('mentor')}><MessageCircle size={14} /> Ментор</button>
        </div>
          ...
          {tab === 'mentor' && (
            <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                {messages.length === 0 && <div style={{ textAlign: 'center', color: '#475569', fontSize: 13, marginTop: 24 }}>Задай вопрос или попроси совета по инвестициям 🐕</div>}
                {messages.map((m) => (
