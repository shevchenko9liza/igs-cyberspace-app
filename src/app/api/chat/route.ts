...
- Чем выше риск — тем дороже страховка (самокат опаснее смартфона, поэтому страховка дороже).
- Управление рисками — это когда ты заранее думаешь о возможных проблемах и готовишься к ним.

### Страхование в России
- В России страхование регулируется законом «Об организации страхового дела в РФ» (№4015-1).
- **ОМС** (обязательное медицинское страхование) — есть у каждого гражданина бесплатно. Покрывает основное лечение.
- **ОСАГО** — обязательное страхование автогражданской ответственности. Без него нельзя ездить на машине.
- **КАСКО** — добровольное страхование автомобиля от повреждений и угона.
- **ДМС** — добровольное медицинское страхование с расширенным перечнем услуг.
- Страховой надзор осуществляет Центральный банк России (Банк России).
- При покупке страховки внимательно читай условия — там указаны исключения (что НЕ покрывается).

### Про Ингосстрах
- Ингосстрах — одна из крупнейших и старейших страховых компаний России, основана в 1947 году.
- Предлагает все основные виды страхования: имущества, здоровья, путешествий, автомобилей, ответственности.
- Работает по принципу митигации рисков — помогает клиентам предотвращать страховые случаи, а не только компенсировать убытки.
- Сайт: ingos.ru — можно оформить полис онлайн.
- У Ингосстраха есть продукты для молодёжи: страхование гаджетов, спортивное страхование, страхование путешественников.
`;

/* ── Fallback-ответы при отсутствии API ────────────────────────── */
const FALLBACK_RESPONSES: Record<string, string> = {
  'скам': 'Скам (мошенничество) — это как раз тот случай, который обычно НЕ покрывается страховкой. Страховка защищает от случайных событий (разбил, украли), но не от обмана. Вывод: будь осторожен в интернете, никакая страховка не спасёт от скама.',
  'чп': 'При ЧП у тебя есть выбор: заплатить всё из кармана или воспользоваться страховкой (если она есть). Со страховкой ты платишь только 20% — это франшиза. Без страховки — 100%. Вот почему страховка важна!',
  'страхов': 'Страхование — это способ защиты от больших финансовых потерь. Ты платишь небольшую сумму (премию), а если происходит страховой случай — компания покрывает убыток. В игре: купи страховку на предмет, и при поломке заплатишь только 20%.',
  'франшиз': 'Франшиза — это часть ущерба, которую ты платишь сам. В нашей игре франшиза 20%. Если ущерб 1000 монет — ты платишь 200, страховая покрывает 800.',
  'кредит': 'Кредит — деньги в долг с процентами. В игре комиссия 15% сразу, и штраф 5% каждые 60 секунд при просрочке. Бери кредит только если уверен, что сможешь быстро вернуть.',
  'долг': 'Долг растёт из-за штрафов — 5% каждые 60 секунд! Если монет не хватает на ремонт, ущерб тоже уходит в долг. Совет: сначала заработай и погаси долг, потом покупай новое.',
  'риск': 'Риск — вероятность поломки. У каждого предмета свой уровень риска. Чем выше риск — тем важнее страховка. Самокат (12%) рискованнее смартфона (8%).',
  'полис': 'Полис — договор со страховой. В нём указано: что застраховано, от чего, сколько платишь и сколько получишь.',
  'инцидент': 'При инциденте два варианта: оплатить всё из кармана или использовать страховку (только 20%). Страховка выгодна при дорогих поломках!',
  'мошен': 'Мошенничество не покрывается страховкой! Страховка работает только при случайных событиях. Скам в играх, фишинг, обман — от этого защищает только осторожность.',
  'обман': 'Обман и мошенничество — это НЕ страховой случай. Страховка покрывает случайные события: поломку, кражу, аварию. Но не ситуации, где тебя обманули.',
};
...
const ALLOWED_TOPICS = [
  ...
  // Игровые инциденты и ситуации
  'чп', 'скам', 'мошен', 'обман', 'сломал', 'разби', 'украл', 'потерял',
  'авари', 'затопи', 'вирус', 'сгорел', 'уронил', 'поступить', 'ситуац',
  'починить', 'починк', 'что делать', 'как быть', 'лучше',
  'телефон', 'смартфон', 'комп', 'самокат', 'планшет', 'ноутбук',
  'жиль', 'квартир', 'дом', 'путешеств', 'поездк', 'спорт',
];
